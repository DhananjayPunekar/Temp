...
# MAGIC %md
# MAGIC # Test: HUB_PLCY (AV)
# MAGIC
# MAGIC Reconciles **HUB_PLCY** (loaded by the framework) against the **expected** records produced by the
# MAGIC reference query `hub_plcy.sql`.
# MAGIC
...
# MAGIC
# MAGIC **Expected query = `hub_plcy.sql` with these adjustments**
...
# MAGIC 2. `PLCY_ID` uses the required key format `<MQP_DISPLAY_POLICY_NUMBER>_<yyyy-MM-ddTHH:mm:ss>`, built exactly as
# MAGIC    `derive_plcy_id` (`derive_concat`, normalize = false) builds it. The plain `CAST ... AS STRING` and the `LOWER`
# MAGIC    in the SQL are not used (`LOWER` would turn the `T` into `t`). The hash follows: `MD5(PLCY_ID || '_109')`.
# MAGIC
# MAGIC **Note**: if other AV pipelines also load HUB_PLCY with `REC_SRC_NM = '109'`, their rows show up as
...
# MAGIC | TC03 | No duplicates on `QBE_HASH_PLCY_ID` and on `PLCY_ID` in the target scope |
# MAGIC | TC04 | Mandatory columns are NOT NULL in the target |
# MAGIC | TC05 | Constant values (`REC_SRC_NM`, `ERR_FLG`, `ERR_CD`) and spec-NULL columns (`PROD_TP`) |
# MAGIC | TC06 | Hash integrity: `QBE_HASH_PLCY_ID = MD5(PLCY_ID || '_109')` on every target row |
# MAGIC | TC07 | Full-row reconciliation (EXCEPT ALL, both directions) |
# MAGIC | TC08 | Key presence and column-level value match (joined on `QBE_HASH_PLCY_ID`) |
...
dbutils.widgets.text("target_table", "HUB_PLCY", "Target (hub) table")
dbutils.widgets.text("source_table", "PLCY_DTL", "Silver source table")
...
SQL_OUTPUT_COLS = ["QBE_HASH_PLCY_ID", "LD_DT", "REC_SRC_NM", "PLCY_ID", "PROD_TP", "ERR_FLG", "ERR_CD"]
...
# PLCY_ID built exactly as derive_plcy_id (derive_concat, normalize=false) builds it: trimmed tokens, blank -> NULL,
# CONCAT_WS('_'), TimestampType tokens formatted yyyy-MM-ddTHH:mm:ss (uppercase 'T').
# The plain CAST(MQP_EFFECTIVE_DATE AS STRING) and the LOWER in hub_plcy.sql are not used.
def derive_concat_token(col_name):
    dtype = col_type(src_df, col_name)
    if isinstance(dtype, T.TimestampType):
        txt = (f"CONCAT(DATE_FORMAT(CAST(S.{col_name} AS TIMESTAMP), 'yyyy-MM-dd'), 'T', "
               f"DATE_FORMAT(CAST(S.{col_name} AS TIMESTAMP), 'HH:mm:ss'))")
    else:
        txt = f"CAST(S.{col_name} AS STRING)"
    return f"NULLIF(TRIM({txt}), '')"

print(f"INFO: MQP_EFFECTIVE_DATE data type = {col_type(src_df, 'MQP_EFFECTIVE_DATE')}")

KEY_EXPR  = (f"CONCAT_WS('_', {derive_concat_token('MQP_DISPLAY_POLICY_NUMBER')}, "
             f"{derive_concat_token('MQP_EFFECTIVE_DATE')})")
HASH_EXPR = f"MD5(CONCAT({KEY_EXPR}, '_{REC_SRC_NM}'))"
EXTRA_EXPRS = {}

# TC03 unique keys and TC06 hash recomputation on the target's own columns.
UNIQUE_KEYS     = [[HASH_KEY], [ID_COL]]
HASH_CHECK_EXPR = f"MD5(CONCAT(CAST(PLCY_ID AS STRING), '_{REC_SRC_NM}'))"


# Source rows in scope of the SQL, before the soft-delete filter.
SCOPE_WHERE = f"""S.MQP_DISPLAY_POLICY_NUMBER IS NOT NULL AND S.MQP_PRODUCT_CODE = '{PRODUCT_CODE}' AND S.MQP_EFFECTIVE_DATE IS NOT NULL"""
...
# The SQL lowercases the whole key; derive_plcy_id keeps the case, so policy numbers with letters differ from the plain SQL.
n_upper = spark.sql(
    f"SELECT COUNT(DISTINCT TRIM(S.MQP_DISPLAY_POLICY_NUMBER)) AS N FROM {SOURCE_FQ} S WHERE {SCOPE_WHERE} "
    f"AND S.{DATE_DELETED_COL} IS NULL AND TRIM(S.MQP_DISPLAY_POLICY_NUMBER) <> LOWER(TRIM(S.MQP_DISPLAY_POLICY_NUMBER))"
).first()["N"]
print(f"INFO: {n_upper} distinct in-scope policy numbers contain uppercase letters (kept as-is in PLCY_ID).")
...
# MAGIC Recomputes `MD5(PLCY_ID || '_109')` from each target row's own columns.
...
# MAGIC %md ## TC08 - Key presence and column-level value match (on `QBE_HASH_PLCY_ID`)
