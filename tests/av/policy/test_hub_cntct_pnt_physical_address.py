...
# MAGIC %md
# MAGIC # Test: HUB_CNTCT_PNT - Physical_Address (AV)
# MAGIC
# MAGIC Reconciles the **`Physical_Address`** rows of **HUB_CNTCT_PNT** (loaded by the framework) against the **expected**
# MAGIC records produced by the reference query `hub_cntct_pnt(physical_address).sql`.
# MAGIC
...
# MAGIC
# MAGIC **Expected query = `hub_cntct_pnt(physical_address).sql` with these adjustments**
...
# MAGIC 2. The `LEFT JOIN hub_cntct_pnt H ... WHERE H.CNTCT_PNT_ID IS NULL` anti-join is removed.
# MAGIC    After the load the hub is populated, so the anti-join would return no rows.
# MAGIC
# MAGIC **Target scope**: `REC_SRC_NM = '109'`, `CNTCT_PNT_TP_CD = 'Physical_Address'` (case-insensitive) and, when the hub has
...
dbutils.widgets.text("source_table", "PHYS_ADDR", "Silver source table")
...
CNTCT_PNT_TP_CD = "Physical_Address"
...
# Business key exactly as in hub_cntct_pnt(physical_address).sql.
KEY_EXPR = """LOWER(CONCAT_WS('_', COALESCE(TRIM(S.MAD_LINE_1), ''), COALESCE(TRIM(S.MAD_LINE_2), ''), COALESCE(TRIM(S.MAD_CITY), ''), COALESCE(TRIM(S.MAD_COUNTY), ''), COALESCE(TRIM(S.MAD_STATE_CODE), ''), COALESCE(TRIM(S.MAD_COUNTRY), ''), COALESCE(TRIM(S.MAD_ZIP_CODE), '')))"""

# Source rows in scope of the SQL, before the soft-delete filter.
SCOPE_WHERE = f"""S.MAD_LINE_1 IS NOT NULL AND S.MQP_PRODUCT_CODE = '{PRODUCT_CODE}'"""
...
# The SQL keeps blank address parts as empty tokens (COALESCE(TRIM(x), '')), e.g. 'line1__city_...'.
# dv_derive_rule.derive_cntct_pnt_phys skips blank parts instead ('line1_city_...'). If the seed uses that
# function, these keys will show up as mismatches in TC02 / TC07 / TC08.
n_blank_tokens = expected_raw.filter(F.col("CNTCT_PNT_ID").rlike(r"(^_|__|_$)")).count()
print(f"INFO: {n_blank_tokens} expected CNTCT_PNT_ID values contain empty address parts ('__', leading or trailing '_').")
