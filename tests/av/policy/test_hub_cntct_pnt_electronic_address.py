...
# MAGIC %md
# MAGIC # Test: HUB_CNTCT_PNT - Electronic_Address (AV)
# MAGIC
# MAGIC Reconciles the **`Electronic_Address`** rows of **HUB_CNTCT_PNT** (loaded by the framework) against the **expected**
# MAGIC records produced by the reference query `hub_cntct_pnt(electronic_address).sql`.
# MAGIC
...
# MAGIC
# MAGIC **Expected query = `hub_cntct_pnt(electronic_address).sql` with these adjustments**
...
# MAGIC
# MAGIC **Target scope**: `REC_SRC_NM = '109'`, `CNTCT_PNT_TP_CD = 'Electronic_Address'` (case-insensitive) and, when the hub has
...
dbutils.widgets.text("source_table", "ELEC_ADDR", "Silver source table")
...
CNTCT_PNT_TP_CD = "Electronic_Address"
...
# Business key exactly as in hub_cntct_pnt(electronic_address).sql.
KEY_EXPR = """LOWER(TRIM(S.MCN_E_MAIL))"""

# Source rows in scope of the SQL, before the soft-delete filter.
SCOPE_WHERE = f"""S.MCN_E_MAIL IS NOT NULL AND S.MQP_PRODUCT_CODE = '{PRODUCT_CODE}' AND TRIM(S.MCN_E_MAIL) <> ''"""
