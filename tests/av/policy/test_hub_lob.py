...
# MAGIC %md
# MAGIC # Test: HUB_LOB (AV)
# MAGIC
# MAGIC Reconciles **HUB_LOB** (loaded by the framework) against the **expected** records produced by the
# MAGIC reference query `hub_lob.sql`.
# MAGIC
...
# MAGIC
# MAGIC **Expected query = `hub_lob.sql` with this adjustment**
...
# MAGIC | TC03 | No duplicate `QBE_HASH_LOB_ID` / `LOB_ID` in the target scope |
...
# MAGIC | TC06 | Hash integrity: `QBE_HASH_LOB_ID = MD5(LOB_ID || '_109')` on every target row |
# MAGIC | TC07 | Full-row reconciliation (EXCEPT ALL, both directions) |
# MAGIC | TC08 | Key presence and column-level value match (joined on `QBE_HASH_LOB_ID`) |
...
dbutils.widgets.text("target_table", "HUB_LOB", "Target (hub) table")
dbutils.widgets.text("source_table", "LOB_DTL", "Silver source table")
...
SQL_OUTPUT_COLS = ["QBE_HASH_LOB_ID", "LD_DT", "REC_SRC_NM", "LOB_ID", "ERR_FLG", "ERR_CD"]
...
# Business key exactly as in hub_lob.sql.
KEY_EXPR = """LOWER(TRIM(S.LOB_CODE))"""

# Source rows in scope of the SQL, before the soft-delete filter.
SCOPE_WHERE = f"""S.LOB_CODE IS NOT NULL AND S.MQP_PRODUCT_CODE = '{PRODUCT_CODE}' AND TRIM(S.LOB_CODE) <> ''"""
...
# The SQL lowercases LOB_CODE; dv_derive_rule.derive_lob_id copies the column without lowercasing.
n_upper = spark.sql(
    f"SELECT COUNT(DISTINCT TRIM(S.LOB_CODE)) AS N FROM {SOURCE_FQ} S "
    f"WHERE {SCOPE_WHERE} AND S.{DATE_DELETED_COL} IS NULL AND TRIM(S.LOB_CODE) <> LOWER(TRIM(S.LOB_CODE))"
).first()["N"]
print(f"INFO: {n_upper} distinct in-scope LOB_CODE values contain uppercase letters (the SQL lowercases them).")
...
# MAGIC Recomputes `MD5(LOB_ID || '_<REC_SRC_NM>')` from each target row's own `LOB_ID`.
...
# MAGIC %md ## TC08 - Key presence and column-level value match (on `QBE_HASH_LOB_ID`)
