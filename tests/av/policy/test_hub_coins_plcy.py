...
# MAGIC %md
# MAGIC # Test: HUB_COINS_PLCY (AV)
# MAGIC
# MAGIC Reconciles **HUB_COINS_PLCY** (loaded by the framework) against the **expected** records produced by the
# MAGIC reference query `hub_coins_plcy.sql`.
# MAGIC
...
# MAGIC
# MAGIC **Expected query = `hub_coins_plcy.sql` with this adjustment**
...
# MAGIC | TC03 | No duplicate `QBE_HASH_COINS_PLCY_ID` / `COINS_PLCY_ID` in the target scope |
# MAGIC | TC04 | Mandatory columns are NOT NULL in the target |
# MAGIC | TC05 | Constant values (`REC_SRC_NM`, `ERR_FLG`, `ERR_CD`) and spec-NULL columns (`QOT_FLG`) |
# MAGIC | TC06 | Hash integrity: `QBE_HASH_COINS_PLCY_ID = MD5(COINS_PLCY_ID || '_109')` on every target row |
# MAGIC | TC07 | Full-row reconciliation (EXCEPT ALL, both directions) |
# MAGIC | TC08 | Key presence and column-level value match (joined on `QBE_HASH_COINS_PLCY_ID`) |
...
dbutils.widgets.text("target_table", "HUB_COINS_PLCY", "Target (hub) table")
dbutils.widgets.text("source_table", "coins_plcy_dtl", "Silver source table")
...
HASH_KEY = "QBE_HASH_COINS_PLCY_ID"
ID_COL   = "COINS_PLCY_ID"
...
SQL_OUTPUT_COLS = ["QBE_HASH_COINS_PLCY_ID", "LD_DT", "REC_SRC_NM", "COINS_PLCY_ID", "QOT_FLG", "ERR_FLG", "ERR_CD"]
...
# Business key exactly as in hub_coins_plcy.sql.
KEY_EXPR = """LOWER(CONCAT(COALESCE(S.RPEC_C_CARRIER, ''), '_', COALESCE(S.RPEC_C_QUOTA_POLICY_NUMBER, ''), '_', COALESCE(CAST(S.RPEC_ID AS STRING), '')))"""

# Source rows in scope of the SQL, before the soft-delete filter.
SCOPE_WHERE = f"""S.RPEC_C_CARRIER IS NOT NULL AND S.MQP_PRODUCT_CODE = '{PRODUCT_CODE}' AND S.RPEC_C_QUOTA_POLICY_NUMBER IS NOT NULL AND S.RPEC_ID IS NOT NULL"""
...
# dv_derive_rule.derive_concat skips blank parts instead; such keys would differ.
n_blank_tokens = expected_raw.filter(F.col("COINS_PLCY_ID").rlike(r"(^_|__|_$)")).count()
print(f"INFO: {n_blank_tokens} expected COINS_PLCY_ID values contain empty key parts ('__', leading or trailing '_').")

# RPEC_ID goes into the key with a plain CAST AS STRING. Fine for numeric/string ids; a timestamp would need yyyy-MM-ddTHH:mm:ss.
_rpec_type = col_type(src_df, "RPEC_ID")
print(f"INFO: RPEC_ID data type = {_rpec_type}")
if isinstance(_rpec_type, T.TimestampType):
    print("WARNING: RPEC_ID is a timestamp; the key format rule (yyyy-MM-ddTHH:mm:ss) is not applied by the SQL.")
...
# MAGIC Recomputes `MD5(COINS_PLCY_ID || '_<REC_SRC_NM>')` from each target row's own `COINS_PLCY_ID`.
...
# MAGIC %md ## TC08 - Key presence and column-level value match (on `QBE_HASH_COINS_PLCY_ID`)
