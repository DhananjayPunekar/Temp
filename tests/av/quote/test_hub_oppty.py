...
# MAGIC %md
# MAGIC # Test: HUB_OPPTY (AV)
# MAGIC
# MAGIC Reconciles **HUB_OPPTY** (loaded by the framework) against the **expected** records produced by the
# MAGIC reference query `hub_oppty.sql`.
# MAGIC
...
# MAGIC
# MAGIC **Expected query = `hub_oppty.sql` with this adjustment**
...
# MAGIC | TC03 | No duplicates on `QBE_HASH_OPPTY_ID` and on `OPPTY_ID` in the target scope |
...
# MAGIC | TC06 | Hash integrity: `QBE_HASH_OPPTY_ID = MD5(OPPTY_ID || '_109')` on every target row |
# MAGIC | TC07 | Full-row reconciliation (EXCEPT ALL, both directions) |
# MAGIC | TC08 | Key presence and column-level value match (joined on `QBE_HASH_OPPTY_ID`) |
...
dbutils.widgets.text("target_table", "HUB_OPPTY", "Target (hub) table")
...
SQL_OUTPUT_COLS = ["QBE_HASH_OPPTY_ID", "REC_SRC_NM", "LD_DT", "OPPTY_ID", "ERR_FLG", "ERR_CD"]
...
# Business key exactly as in av/quotes/hub_oppty.sql: OPPTY_ID = LOWER(TRIM(MQP_C_OPPORTUNITY_ID)) || '_1'.
# The SQL hash MD5(LOWER(TRIM(id)) || '_1_109') is the same as MD5(OPPTY_ID || '_109').
KEY_EXPR  = "CONCAT(LOWER(TRIM(CAST(S.MQP_C_OPPORTUNITY_ID AS STRING))), '_1')"
...
HASH_CHECK_EXPR = f"MD5(CONCAT(CAST(OPPTY_ID AS STRING), '_{REC_SRC_NM}'))"


# Source rows in scope of the SQL, before the soft-delete filter.
SCOPE_WHERE = f"""S.MQP_C_OPPORTUNITY_ID IS NOT NULL AND TRIM(S.MQP_C_OPPORTUNITY_ID) <> '' AND S.MQP_PRODUCT_CODE = '{PRODUCT_CODE}'"""
...
# One opportunity is usually shared by several quotes; the hub keeps one row per opportunity.
n_src_rows = spark.sql(f"SELECT COUNT(*) AS N FROM {SOURCE_FQ} S WHERE {SCOPE_WHERE} AND S.{DATE_DELETED_COL} IS NULL").first()["N"]
print(f"INFO: {n_src_rows} in-scope quote rows collapse to the distinct opportunities in the expected data.")
...
# MAGIC Recomputes `MD5(OPPTY_ID || '_109')` from each target row's own columns.
...
# MAGIC %md ## TC08 - Key presence and column-level value match (on `QBE_HASH_OPPTY_ID`)
