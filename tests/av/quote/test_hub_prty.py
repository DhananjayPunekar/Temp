...
# MAGIC %md
# MAGIC # Test: HUB_PRTY - quote parties (AV)
# MAGIC
...
# MAGIC A policy business name that itself ends in `_1` would still fall into this scope.
# MAGIC
...
dbutils.widgets.dropdown("only_quote_parties", "Y", ["Y", "N"], "Limit target scope to PRTY_ID ending in _1")
...
# Business key, party type and hash exactly as in av/quotes/hub_prty.sql:
#   PRTY_ID    = LOWER(TRIM(BUSINESSNAME)) || '_1'
#   PRTY_TP_CD = TRIM(PARTY_TYPE)            (no Person / Organization mapping)
#   hash       = MD5(CONCAT_WS('_', PRTY_ID, LOWER(PRTY_TP_CD), '109'))
KEY_EXPR  = "CONCAT(LOWER(TRIM(S.BUSINESSNAME)), '_1')"
HASH_EXPR = f"MD5(CONCAT_WS('_', {KEY_EXPR}, LOWER(TRIM(S.PARTY_TYPE)), '{REC_SRC_NM}'))"
EXTRA_EXPRS = {"PRTY_TP_CD": "TRIM(S.PARTY_TYPE)"}

# TC03 unique keys (one business name can exist with several party types)
# and TC06 hash recomputation on the target's own columns.
UNIQUE_KEYS     = [[HASH_KEY], [ID_COL, "PRTY_TP_CD"]]
HASH_CHECK_EXPR = f"MD5(CONCAT_WS('_', CAST(PRTY_ID AS STRING), LOWER(TRIM(PRTY_TP_CD)), '{REC_SRC_NM}'))"
...
# PRTY_TP_CD keeps the source case, but the hash lowercases it: 'Person' and 'PERSON' for the same name give
# two expected rows with the same hash. A hub can hold only one of them.
n_case_dups = expected_raw.groupBy(HASH_KEY).count().filter("count > 1").count()
print(f"INFO: {n_case_dups} expected {HASH_KEY} values appear with more than one PRTY_TP_CD spelling (case variants).")
...
if dbutils.widgets.get("only_quote_parties") == "Y":
    actual_raw = actual_raw.filter(F.col("PRTY_ID").endswith("_1"))
    print("Target scope limited to quote parties (PRTY_ID ending in '_1').")
