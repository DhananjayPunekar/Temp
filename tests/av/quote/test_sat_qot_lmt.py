...
# MAGIC %md
# MAGIC # Test: AV__GOLD__SAT_QOT_LMT
# MAGIC
# MAGIC Reconciles **SAT_QOT_LMT** (loaded by the framework from `seed_sat_qot_lmt.sql`) against the
# MAGIC **expected** records produced by the reference query `sat_qot_lmt.sql`.
# MAGIC
# MAGIC Only the columns in the SQL query output are validated; other target columns (`PART_COL`, ...) are ignored.
# MAGIC
# MAGIC **Expected query = `sat_qot_lmt.sql` with these adjustments**
...
# MAGIC 2. `QL.DATE_DELETED IS NULL` is added. Every silver table carries `DATE_DELETED` and the framework drops
# MAGIC    soft-deleted rows before any join.
# MAGIC 3. `QL.MQP_PRODUCT_CODE = '<product_code>'` is added, matching `product_filter_col` in the seed
# MAGIC    (the reference SQL has no product filter). Controlled by the `apply_product_filter` widget.
# MAGIC 4. The `LEFT JOIN SAT_QOT_LMT ... WHERE SQLMT.QBE_HASH_QOT_ID IS NULL` anti-join is removed.
...
# MAGIC | TC05 | NULL-count parity per column (covers spec-NULL columns such as `LMT_CD`) |
...
dbutils.widgets.text("target_table", "SAT_QOT_LMT", "Target table")
dbutils.widgets.text("hub_table", "HUB_QOT", "Hub table")
dbutils.widgets.text("source_table", "QOT_LMT", "Silver source table")
dbutils.widgets.text("rec_src_nm", "109", "REC_SRC_NM")
dbutils.widgets.text("product_code", "AV", "Product code")
dbutils.widgets.text("compare_key", "QBE_HASH_QOT_ID,LMT_TP_CD", "Compare key (comma separated)")
dbutils.widgets.dropdown("apply_product_filter", "Y", ["Y", "N"], "Filter source on MQP_PRODUCT_CODE (seed product_filter_col)")
dbutils.widgets.dropdown("validate_batch_cols", "Y", ["Y", "N"], "Validate BTCH_ID / BTCH_DT values")
...
PRODUCT_FILTER = dbutils.widgets.get("apply_product_filter") == "Y"
VALIDATE_BATCH = dbutils.widgets.get("validate_batch_cols") == "Y"
...
print(f"REC_SRC_NM={REC_SRC_NM}  product={PRODUCT_CODE}  product_filter={PRODUCT_FILTER}  compare_key={COMPARE_KEY}")
...
SQL_OUTPUT_COLS = [
    "QBE_HASH_QOT_ID", "LMT_TP_CD", "LD_DT", "REF_ID", "LD_END_DT", "REC_SRC_NM",
    "LMT_DATA_TP_CD", "LMT_BSIS_CD", "LMT_APLY_TO_CD", "LMT_AMT", "LMT_CD",
    "SRC_EFF_DT", "SRC_EXPRN_DT", "ERR_FLG", "ERR_CD", "BTCH_ID", "BTCH_DT",
]

# LD_DT is CURRENT_TIMESTAMP at run time, so only NOT NULL is checked, never its value.
RUNTIME_COLS = ["LD_DT"]

# BTCH_ID / BTCH_DT come straight from the silver row. The framework does not re-insert a record that
# reappears in a later silver batch, so these can legitimately differ when the source holds the same
# record in several batches. Set 'validate_batch_cols' = N to only check them for NOT NULL.
BATCH_COLS = ["BTCH_ID", "BTCH_DT"]
if not VALIDATE_BATCH:
    RUNTIME_COLS = RUNTIME_COLS + BATCH_COLS
...
MANDATORY_COLS = ["QBE_HASH_QOT_ID", "LD_DT", "REF_ID", "REC_SRC_NM", "SRC_EFF_DT", "ERR_FLG", "ERR_CD", "BTCH_ID", "BTCH_DT"]
...
def derive_concat_token(col_name):
    ...
    if isinstance(dtype, T.TimestampType):
        txt = (f"CONCAT(DATE_FORMAT(CAST(QL.{col_name} AS TIMESTAMP), 'yyyy-MM-dd'), 'T', "
               f"DATE_FORMAT(CAST(QL.{col_name} AS TIMESTAMP), 'HH:mm:ss'))")
    else:
        txt = f"CAST(QL.{col_name} AS STRING)"
...
product_filter = f"AND QL.MQP_PRODUCT_CODE = '{PRODUCT_CODE}'" if PRODUCT_FILTER else ""
...
WITH QOT_LMT_SRC AS (
    ...
    WHERE QL.MQP_ENTITY_REFERENCE IS NOT NULL
      ...
      AND QL.{DATE_DELETED_COL} IS NULL
      {product_filter}
),
LINKED AS (
...
scope_df = src_df.filter(F.col("MQP_PRODUCT_CODE") == PRODUCT_CODE) if PRODUCT_FILTER else src_df
...
# MAGIC Several rows can share one `QBE_HASH_QOT_ID` and `SRC_EFF_DT` (one per `LMT_TP_CD`), so which row is "latest"
...
# MAGIC A target row whose (`REF_ID`, `SRC_EFF_DT`, `LMT_TP_CD`) only exists in soft-deleted source rows means
# MAGIC the framework did not apply the `DATE_DELETED` filter.

# COMMAND ----------

_src_key = ["MQP_ENTITY_REFERENCE", "MQP_DATE_MODIFIED", "LMT_TYPE"]
_tgt_key = ["REF_ID", "SRC_EFF_DT", "LMT_TP_CD"]
