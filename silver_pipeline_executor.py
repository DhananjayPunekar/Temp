...
class PipelineExecutor:
    ...
    def execute(self, cfg, settings, context, load_type, layer="silver"):
        ...
        try:
            ...
            _default_partition_cols = ["REC_SRC_NM", "PART_COL"]
            _partition_candidates = _partition_by_cfg or _default_partition_cols
            _partition_by_actual = [c for c in _partition_candidates if c in df.columns]
            if not _partition_by_cfg and _partition_by_actual:
                logger.info(
                    "[PipelineExecutor] Using default partition/merge scope columns for pipeline %s: %s",
                    pid,
                    _partition_by_actual,
                )
            logger.info(
                "[PipelineExecutor] STAGE_START %s",
                _fmt_vars(
            ...
            result = writer.write(
                ...
                watermark_col=cfg.get("watermark_col"),
                rank_order_cols=cfg.get("scd2_rank_order_cols"),
                product_code=context.get("products"),
            )
